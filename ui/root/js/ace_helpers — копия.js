...
i18n.addPostProcessor("localizeEntityName", function(value, key, isFound, opts) {
   ...
   opts.postProcess = null;
	...
	function endModify(word) //Функция изменения окончания
	{
		...
		const m_adjectives_i = ["гий-",		"цкий-",	"кий-",		"хий-",		"ой-",	"ый-",	"ий-"]; //Мужицкие прилагательные
		const m_adjectives_r = ["гого-",	"цкого-",	"кого-",	"хого-",	"ого-",	"ого-",	"его-"];
		const m_adjectives_d = ["гому-",	"цкому-",	"кому-",	"хому-",	"ому-",	"ому-",	"ему-"];
		const m_adjectives_v = ["гого-",	"цкий-",	"кого-",	"хого-",	"ой-",	"ый-",	"ий-"];
		const m_adjectives_t = ["гим-",		"цким-",	"ким-",		"хим-",		"ым-",	"ым-",	"им-"];
		const m_adjectives_p = ["гом-",		"цком-",	"ком-",		"хом-",		"ом-",	"ом-",	"ем-"];
		const m_adjectives_x = ["гие-",		"цкие-",	"кие-",		"хие-",		"ие-",	"ые-",	"ие-"];
		...
		const f_adjectives_v = ["ой-",	"ый-",	"ий-",	"юю-"];
		...
		const m_nouns_i = ["-"]; //Мужицкие существительные
		const m_nouns_r = ["а-"];
		const m_nouns_d = ["у-"];
		const m_nouns_v = ["-"];
		const m_nouns_t = ["ом-"];
		...
		const x_nouns_v = ["ги-",	"ков-",		"хов-",		"и-",	"ов-"];
		...
		while (Text.indexOf(endModifyPref) != -1) 
		{
			...
			*/ 
			...
			for (var i = 0; i < tokensLength; i++) //проходим по каждому слову
			{
				...
				var isNumber = 0; //Является ли слово числом
				...
				if ( firstCharCode >= 1025 && firstCharCode <= 1103) //Если первый символ слова является кириллицей (нам нужно склонять только русские слова)
				{
					...
					for (var e = 0; e < adjectivesLength; e++) //проверяем окончание части слова, проходя по окончаниям заданных массивов
					{
						if (tokens[i].indexOf(adjectives_i[e]) >= 0) //если это прилагательное, и окончание подходит...
						{
							switch (suffix) //Заменяем окончание в зависимости от падежа
							{
								case 'r':
									tokens[i] = tokens[i].replace(adjectives_i[e], adjectives_r[e]); break;
							
								case 'd':
									tokens[i] = tokens[i].replace(adjectives_i[e], adjectives_d[e]); break;
								
								case 'v':
									tokens[i] = tokens[i].replace(adjectives_i[e], adjectives_v[e]); break;
								
								case 't':
									tokens[i] = tokens[i].replace(adjectives_i[e], adjectives_t[e]); break;
								
								case 'p':
									tokens[i] = tokens[i].replace(adjectives_i[e], adjectives_p[e]); break;
							}
							success = 1;
					...
					if (!success && ((nouns < maxNouns) || (maxNouns == 0))) 
					{
						...
						{
							parts[t] = parts[t] + '-';
							for (var e = 0; e < nounsLength; e++) //проверяем окончание части слова, проходя по окончаниям заданных массивов
							{
								if (parts[t].indexOf(nouns_i[e]) >= 0) //если это существительное, и окончание подходит...
								{
									switch (suffix) //Заменяем окончание в зависимости от падежа
									{
										case 'r':
											parts[t] = parts[t].replace(nouns_i[e], nouns_r[e]); break;
									
										case 'd':
											parts[t] = parts[t].replace(nouns_i[e], nouns_d[e]); break;
										
										case 'v':
											parts[t] = parts[t].replace(nouns_i[e], nouns_v[e]); break;
										
										case 't':
											parts[t] = parts[t].replace(nouns_i[e], nouns_t[e]); break;
										
										case 'p':
											parts[t] = parts[t].replace(nouns_i[e], nouns_p[e]); break;
									}
									nouns++;
